...
function setActiveNavLink() {
  ...
  $(".navbar__link, .navbar__mobile-link").each(function () {
    ...
    const isActive =
      hrefPage === path || (path === "" && hrefPage === "index.html");
...
function initGlobalBookmarks() {
  $(document).on("click", ".recipe-card__bookmark", function (e) {
    ...
    $btn
      .find("i")
      .attr("class", isNowSaved ? "fas fa-bookmark" : "far fa-bookmark");
    $btn.attr({
    ...
    setTimeout(function () {
      $btn.removeClass("animate-scaleIn");
    }, 400);
...
/**
 * Global click listener for Recipe Cards
 */
function initRecipeCardClicks() {
  $(document).on("click", ".recipe-card", function (e) {
    if ($(e.target).closest(".recipe-card__bookmark, a, button").length) return;
